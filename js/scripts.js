...
window.addEventListener('DOMContentLoaded' , () => {
    ...
    const tabsContentMenu = document.querySelectorAll('.tab-content');
    const tabsMenuList = document.querySelectorAll('.tabs-menu > li');
    const tabsMenuLink = document.querySelectorAll('.tabs-menu > li > a');

    function hideTabContentMenu (){
        tabsContentMenu.forEach(item => {
    ...
    function showTabContantMenu(){
        for(let i = 0; i < tabsMenuLink.length; i++){
            tabsMenuLink[i].addEventListener('click' , function (){
                hideTabContentMenu ();
                tabsMenuList[i].classList.add('current');
                tabsContentMenu[i].classList.add('first-tab');
            })
        }
    }
    ...
    const swiperSlidesEvents = document.querySelectorAll('.events-carousel > .swiper-container > .swiper-wrapper > .swiper-slide');
    const prevEventButton = document.querySelector('.ec-button-prev');
    const nextEventButton = document.querySelector('.ec-button-next');
    let swiperEventIndex = 0;

    function swiperSlideChange(index){
        for(let i = 0; i < swiperSlidesEvents.length; i++){
            swiperSlidesEvents[i].style.display = 'none';
        }

        swiperSlidesEvents[index].style.display = 'block';

    }
    function prevEvent(n){
        swiperEventIndex += n;

        if(swiperEventIndex < 0){
            swiperEventIndex = swiperSlidesEvents.length - 1;
        }

        if(swiperEventIndex > swiperSlidesEvents.length - 1){
            swiperEventIndex = 0;
        }

        swiperSlideChange(swiperEventIndex);
    }
    ...
    // Home testimonilas-carousel


    const swiperSlidesTestimonilas = document.querySelectorAll('.testimonilas-carousel > .swiper-container > .swiper-wrapper > .swiper-slide');
    const tcButtonPrev = document.querySelector('.tc-button-prev');
    const tcButtonNext = document.querySelector('.tc-button-next');
    let indexSlidesTestimonilas = 0;

    function swiperSlidesTestimonilasChange(index){
        swiperSlidesTestimonilas.forEach(item => {
            item.style.display = 'none';
        })
        swiperSlidesTestimonilas[index].style.display = 'block';
    }

    function TestimonilasIndexChange(n){

        indexSlidesTestimonilas += n;

        if(indexSlidesTestimonilas < 0){
            indexSlidesTestimonilas = swiperSlidesTestimonilas.length - 1;
        }

        if(indexSlidesTestimonilas > swiperSlidesTestimonilas.length - 1){
            indexSlidesTestimonilas = 0;
        }

        swiperSlidesTestimonilasChange(indexSlidesTestimonilas);
    }

    tcButtonPrev.addEventListener('click' , function(){
        TestimonilasIndexChange(-1);
    });

    tcButtonNext.addEventListener('click' , function(){
        TestimonilasIndexChange(1);
    })
})